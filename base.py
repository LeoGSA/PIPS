...
class Base:
	...
	def timeit(method):
		"""декоратор для замера времени выполнения финкций"""
	...
	def if_pics(u_id, ph_id):
		...
		f.execute("SELECT * FROM Pics WHERE ID='%s' AND Pic='%s'" % (u_id, ph_id))
		u = f.fetchone()
	...
	# @timeit
	def if_piclist(ph_id_list): # в разработке
		"""проверяет, есть фотки из списка в общей базе, создает и возвращает новый список тех, которых нет"""
		...
		for i in ph_id_list:
			f.execute("SELECT ID FROM Pics WHERE Pic='%s'" % str(i))
			u = f.fetchone()
			if u != None:
				list1.append(i)
		return list1
